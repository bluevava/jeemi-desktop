...
需要使用通用 URI 订阅时，推荐按 [通用 URI 订阅结构解析说明文档](URI_SUBSCRIPTION.md) 提供的统一结构生成。文档包含当前支持的协议、参数、编码规则和完整示例，支持明文列表或整份列表的单层 Base64 编码。
...
其他客户端的部分 URI 参数别名、旧式 SS 链接以及 Surge 配置也提供兼容解析，但不推荐作为新订阅的输出格式。兼容范围以说明文档和当前实现为准，不代表完整支持其他客户端的所有协议、路由或私有字段。
